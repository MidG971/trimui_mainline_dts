...
| `trimui-cpu-opp.dtsi` | Per-cluster CPU DVFS (`cpufreq-dt` OPP tables, little 408–1416 / big 408–1800 MHz + commented turbo rows) + `cpu-supply` + dynamic-power-coeff | Nothing external — both former blockers resolved **locally**: CPU-CCU adopted from ut-slayer (`0015-0020`, `&cpu_ccu` CLK_CPUL/CPUB), and the big-cluster CPU supply is **axp1530@0x36** (`&reg_ext_dcdc1`, already mainline `x-powers,axp1530`) — confirmed on HW; our earlier `tcs4838` (patch `0014`) was the wrong chip and was dropped. Builds today. Already board-only (no `-upstream` variant needed — see the file header). | in use now (opt-in `#include`); enable + tune voltages on HW. **When the CPU-CCU is upstreamed** (ut-slayer's to submit): drop `0015-0020` and use this file unchanged — *unless* the binding is renamed in review (watch CLK_CPUL/CPUB + compatible) |
